function initializeSystem(){
  var headers={
    ...
    REPORTS:['id','studentId','classId','academicYearId','term','startDate','endDate','status','version','regeneratedFromId','sourceEvaluationIds','formVersionsJson','summaryJson','evaluationCount','templateId','templateVersion','documentFileId','pdfFileId','generatedBy','generatedAt','createdAt','updatedAt'],
  ...
  if(!c.some(function(x){return x.key===CONFIG.ACTIVE_REPORT_TEMPLATE_ID_KEY;}))DatabaseService.append('CONFIG',{key:CONFIG.ACTIVE_REPORT_TEMPLATE_ID_KEY,value:''});
  if(!c.some(function(x){return x.key===CONFIG.ACTIVE_REPORT_TEMPLATE_VERSION_KEY;}))DatabaseService.append('CONFIG',{key:CONFIG.ACTIVE_REPORT_TEMPLATE_VERSION_KEY,value:'1.0'});
